  ...
  return (
    <section id="price" className="mx-auto w-full max-w-md px-4 py-6 sm:px-6 sm:py-8">
      <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card p-7 text-center">
        <span className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Price per person</span>

        <div className="flex items-baseline gap-2">
          <span className="font-display text-4xl font-bold tracking-tight text-foreground">{formatINR(displayPrice)}</span>
          {discountedPrice ? (
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Reserve with just <span className="font-semibold text-foreground">{formatINR(bookingAmount)}</span> — pay the rest before departure.
        </p>

        <Link
          href={bookHref}
          className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-primary px-6 py-3 text-sm font-bold text-primary-foreground transition-colors hover:bg-primary/90 sm:text-base"
        >
          Reserve Your Seat
        </Link>
