  ...
  return (
    <TiltCard>
      ...
      >
        ...
        >
          <TripImage asset={destination.coverImage} theme={theme} variant="cover" containerClassName="h-full w-full rounded-none" />
        </motion.div>
 ...
 *
 * Continuously-scrolling strip (`MarqueeCarousel`, same engine as
 * `FeaturedTripsSection` above it) rather than a step-and-pause slider —
 * cards drift left in one unbroken loop and pause on hover/touch. Each card
 * gets the same fixed footprint width as a trip card above, so the two
 * sections line up even though their content looks different.
 *
...
export function ThemeExplorerSection({ destinations }: ThemeExplorerSectionProps) {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-section-sm sm:py-section-md">
      ...
      <MarqueeCarousel label="Explore by destination" durationSeconds={Math.max(12, destinations.length * 4)}>
        {destinations.map((destination) => (
          <div key={destination.slug} className="w-64 shrink-0 sm:w-72">
